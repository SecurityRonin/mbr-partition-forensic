...
fn identify_all_zeros_boot_code() {
    let code = [0u8; 446];
    assert_eq!(
        mbr_partition_forensic::boot_code::identify(&code),
        BootCodeId::AllZeros
    );
}
...
fn all_kinds() -> Vec<AnomalyKind> {
    vec![
        ...
        AnomalyKind::WipedRegion {
            lba_start: 11,
            pattern: mbr_partition_forensic::wipe::FillPattern::Ones,
        },
...
fn walk_ebr_chain_byte_offset_overflow_terminates() {
    ...
    let chain = mbr_partition_forensic::ebr::walk_ebr_chain(&mut empty, 2, u64::MAX).unwrap();
    assert!(chain.entries.is_empty());
    assert!(!chain.had_cycle);
    assert!(!chain.depth_exceeded);
}
...
fn walk_ebr_chain_next_pointer_overflow_terminates() {
    ...
    let chain = mbr_partition_forensic::ebr::walk_ebr_chain(&mut disk, u64::MAX - 1, 1).unwrap();
